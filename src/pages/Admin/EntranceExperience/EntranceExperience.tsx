...
const validationSchema = Yup.object().shape({
  ...
  profile_questions: Yup.array<ProfileQuestion>()
    ...
    .defined()
    .transform((val) =>
      val.filter((s: Question) => !!s.name && !!s.text)
    ),
  entrance: Yup.array(
    Yup.object().shape({
...
const EntranceExperience: React.FC<EntranceExperienceProps> = ({ venue, onSave }) => {
  const {
    formState: { dirty },
    register,
    handleSubmit,
  ...
  const onSubmit = useCallback(async (data: Venue_v2_EntranceConfig) => {
    if (!user) return;
