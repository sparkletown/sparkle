...
const validationSchema = Yup.object()
  .shape<Venue_v2_AdvancedConfig>({
    showGrid: Yup.boolean().notRequired(),
    columns: Yup.number().when(
      'showGrid',
      {
        is: true,
    ...
    >
      <Form.Group>
        ...
        <Form.Control
          ...
          min={1}
        />
        {errors.columns && (<span className="input-error">{errors.columns.message}</span>)}
      </Form.Group>
    </ToggleElement>
  );
  ...
  const renderRadioToggle = () => (
    ...
    >
      <Form.Group>
        ...
        />
        {errors.radioStations && (<span className="input-error">{errors.radioStations.message}</span>)}
      </Form.Group>
    </ToggleElement>
  );
  ...
  const renderRoomVisibility = () => (
    <S.ItemWrapper>
      <S.ItemHeader>
        ...
        <S.ItemSubtitle>Choose how you&apos;d like your rooms to appear on the map</S.ItemSubtitle>
      </S.ItemHeader>
