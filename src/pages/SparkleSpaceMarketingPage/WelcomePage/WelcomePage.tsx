...
interface InterestedPersonEmail {
  email: string;
}

const WelcomePage: React.FunctionComponent = () => {
  const { register, handleSubmit, setValue } = useForm<InterestedPersonEmail>();
  const firebase = useFirebase();

  const onSubmit = async (data: InterestedPersonEmail) => {
    await firebase.firestore().collection("interestedPerson").add(data);
    setValue("email", "");
  };
  return (
    <>
