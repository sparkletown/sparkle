...
class PlayerIOWrapper {
  ...
  constructor() {
    (function (wrapper) {
      wrapper._pio.channel = function () {};
      wrapper._pio.channel.prototype.call = function (
        method,
        args,
        successCallback,
        errorCallback,
        ...
        var url =
          typeof PLAYERIO_API_HOST != "undefined"
            ? PLAYERIO_API_HOST
            : (wrapper.PlayerIO.useSecureApiRequests ? "https" : "http") +
              "://api.playerio.com/json/";

        var webrequest = new XMLHttpRequest();
        if ("withCredentials" in webrequest) {
          // Both Safari 4 and Firefox 3.5 provide the withCredentials property on XMLHttpRequest
          //webrequest.withCredentials = true;
          webrequest.open("post", url, true);
        } else if (typeof XDomainRequest != "undefined") {
          // IE uses XDomainRequest for some reason (?)
          webrequest = new XDomainRequest();
          webrequest.open("post", url);
        } else {
          // USE FLASH
          webrequest = new wrapper._pio.flashWebRequest("post", url);
        }
        ...
        if (webrequest != null) {
          webrequest.send(
            "[" + method + "|" + (this.token || "") + "]" + JSON.stringify(args)
          );
          webrequest.onload = function () {
            ...
            try {
              ...
              wrapper._pio.handleError(
                originalStack,
                errorCallback,
                wrapper.PlayerIOErrorCode.GeneralError,
                "Error decoding response from webservice: " + e
              );
              return;
            }
          ...
          webrequest.onerror = function (req) {
            wrapper._pio.handleError(
              ...
              errorCallback,
              wrapper.PlayerIOErrorCode.GeneralError,
              "Error talking to webservice: " + JSON.stringify(req)
            );
          };
        } else {
          wrapper._pio.handleError(
      ...
      wrapper._pio.runCallback = function (
        callback,
        callbackArg,
        originalStackError
      ) {
        try {
          ...
          if (originalStackError) {
            message += "\nCallsite stack:\n";
            message +=
              originalStackError.stack ||
              originalStackError.stacktrace ||
              originalStackError.StackTrace;
          }
          console.log(message);
        }
      };
      wrapper._pio.handleError = function (
        originalStackError,
        errorCallback,
        code,
        message
      ) {
        var err = wrapper._pio.error(code, message);
        if (originalStackError) {
          if (originalStackError.stack) err.stack = originalStackError.stack;
          if (originalStackError.stacktrace)
            err.stacktrace = originalStackError.stacktrace;
          if (originalStackError.StackTrace)
            err.StackTrace = originalStackError.StackTrace;
        }

        if (errorCallback) {
          wrapper._pio.runCallback(errorCallback, err, originalStackError);
      ...
      wrapper._pio.error = function (code, message) {
        if (arguments.length == 1) {
          message = code;
          code = wrapper.PlayerIOErrorCode.GeneralError;
        }

        if (typeof code == "number") {
          code = wrapper.PlayerIOErrorCode.codes[code];
        }
        ...
        var e = new Error();
        return new wrapper.PlayerIOError(
          code,
          message,
          e.stack || e.stacktrace || e.StackTrace
        );
      };
      wrapper._pio.debugLog = function (message) {
        if (typeof console != "undefined") {
          console.log(message);
        }
      };

      wrapper._pio.convertToKVArray = function (object) {
        var result = [];
        if (object) {
          for (var k in object) {
      ...
      wrapper._pio.convertFromKVArray = function (arr) {
        var result = {};
        if (arr && arr.length) {
          for (var k in arr) {
      ...
      wrapper._pio.convertToSegmentArray = function (object) {
        var result = [];
        if (object) {
          for (var k in object) {
     ...
     *	},
     *  function(error) {
     *		if (error.code == wrapper.PlayerIOErrorCode.UnknownGame) {
    ...
    this.PlayerIO = {
       ...
       * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs
       */
      authenticate: function (
        ...
        var channel = new this._pio.channel();
        channel.authenticate(
          gameId,
          connectionId,
          this._pio.convertToKVArray(authenticationArguments),
          this._pio.convertToSegmentArray(playerInsightSegments),
          "javascript",
          this._pio.convertToKVArray({}),
          null,
          successCallback,
          errorCallback,
          function (result) {
            channel.token = result.token;
            return new this._pio.client(
              channel,
              gameId,
              result.gamefsredirectmap,
              result.userid
            );
          }.bind(this)
        );
      ...
      gameFS: function (gameId) {
        return new wrapper._pio.gameFS(gameId);
      },
    };
    var JSON;
    if (!JSON) {
    ...
    (function (wrapper) {
      ...
      // this method gets called back from flash TODO: is need???
      // _wrapper._pio_flashfallback_callback__ = function () {
      ...
      wrapper._pio.flashWebRequest = function (method, url) {
        var self = this;
      ...
      wrapper._pio.flashSocketConnection = function (
        endpoint,
        connectTimeout,
        onConnectResult,
        onDisconnect,
        ...
        var id = "cb" + idCounter++;
        var self = this;
        var serializer = new wrapper._pio.messageSerializer();
          ...
          if (obj == null) {
            ...
            callbacks[id] = function (evt, data) {
              switch (evt) {
                ...
                case "onmessage":
                  onMessage(
                    serializer.deserializeMessage(data, 0, data.length)
                  );
                  break;
              }
            };
      ...
      wrapper._pio.isFlashFallbackEnabled = function (callback) {
        getFlashFallbackObj(function (obj) {
          callback(obj != null);
        });
      };
      ...
      function createFallbackObj() {
        var id = "_wrapper._pio_flashfallback__";
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        var containerId = "_wrapper._pio_flashfallback_container__";
        var html = "";
        html +=
          '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" width="10" height="10" style="$style$" id="$id$">';
        html += '	<param name="movie" value="$src$" />';
        html += '	<param name="allowNetworking" value="all" />';
        html += '	<param name="allowScriptAccess" value="always" />';
        html += "	<!--[if !IE]>-->";
        html +=
          '	<object type="application/x-shockwave-flash" data="$src$" width="10" height="10" style="$style$">';
        html += '		<param name="allowNetworking" value="all" />';
        html += '		<param name="allowScriptAccess" value="always" />';
        html += "	</object>";
        html += "	<!--<![endif]-->";
    ...
    (function (wrapper) {
      var c = wrapper._pio.channel.prototype;
      c.connect = function (
        gameId,
        connectionId,
        userId,
      ...
      wrapper._pio.LockType = { NoLocks: 0, LockIndividual: 1, LockAll: 2 };
      ...
      c.loadMatchingObjects = function (
        table,
        index,
        indexValue,
        limit,
      ...
      c.payVaultReadHistory = function (
        page,
        pageSize,
        targetUserId,
        successCallback,
      ...
      c.payVaultPaymentInfo = function (
        provider,
        purchaseArguments,
        items,
        successCallback,
      ...
      c.payVaultUsePaymentInfo = function (
        provider,
        providerArguments,
        successCallback,
        errorCallback,
      ...
      c.notificationsRefresh = function (
        lastVersion,
        successCallback,
        errorCallback,
        converter
      ...
      c.notificationsRegisterEndpoints = function (
        lastVersion,
        endpoints,
        successCallback,
        errorCallback,
      ...
      c.notificationsToggleEndpoints = function (
        lastVersion,
        endpoints,
        enabled,
        successCallback,
      ...
      c.notificationsDeleteEndpoints = function (
        lastVersion,
        endpoints,
        successCallback,
        errorCallback,
      ...
      c.gameRequestsRefresh = function (
        playCodes,
        successCallback,
        errorCallback,
        converter
      ...
      c.achievementsRefresh = function (
        lastVersion,
        successCallback,
        errorCallback,
        converter
      ...
      c.achievementsProgressSet = function (
        achievementId,
        progress,
        successCallback,
        errorCallback,
      ...
      c.achievementsProgressAdd = function (
        achievementId,
        progressDelta,
        successCallback,
        errorCallback,
      ...
      c.achievementsProgressMax = function (
        achievementId,
        progress,
        successCallback,
        errorCallback,
      ...
      c.achievementsProgressComplete = function (
        achievementId,
        successCallback,
        errorCallback,
        converter
      ...
      c.playerInsightRefresh = function (
        successCallback,
        errorCallback,
        converter
      ) {
      ...
      c.playerInsightSetSegments = function (
        segments,
        successCallback,
        errorCallback,
        converter
      ...
      c.playerInsightTrackInvitedBy = function (
        invitingUserId,
        invitationChannel,
        successCallback,
        errorCallback,
      ...
      c.playerInsightTrackEvents = function (
        events,
        successCallback,
        errorCallback,
        converter
      ...
      c.playerInsightTrackExternalPayment = function (
        currency,
        amount,
        successCallback,
        errorCallback,
      ...
      c.playerInsightSessionKeepAlive = function (
        successCallback,
        errorCallback,
        converter
      ) {
      ...
      c.playerInsightSessionStop = function (
        successCallback,
        errorCallback,
        converter
      ) {
      ...
      c.simpleRecoverPassword = function (
        gameId,
        usernameOrEmail,
        successCallback,
        errorCallback,
      ...
      c.facebookOAuthConnect = function (
        gameId,
        accessToken,
        partnerId,
        playerInsightSegments,
      ...
      c.simpleUserGetSecureLoginInfo = function (
        successCallback,
        errorCallback,
        converter
      ) {
      ...
      c.webserviceOnlineTest = function (
        successCallback,
        errorCallback,
        converter
      ) {
     ...
     * </listing>
     */
    this.PlayerIOError = function (code, message, stack) {
      ...
      this.toString = function () {
        return "wrapper.PlayerIOError[" + code + "]: " + message;
      };
    };
    this.PlayerIOError.prototype = new Error();
    ...
    (function (wrapper) {
      ...
      wrapper._pio.client = function (
         ...
         * @type gameFS
         */
        this.gameFS = new wrapper._pio.gameFS(gameId, gameFsRedirectMap);

        /**
         * The ErrorLog service
         * @type errorLog
         */
        this.errorLog = new wrapper._pio.errorLog(channel);

        /**
         * The PayVault service
         * @type payVault
         */
        this.payVault = new wrapper._pio.payVault(channel);

        /**
         * The BigDB service
         * @type bigDB
         */
        this.bigDB = new wrapper._pio.bigDB(channel);

        /**
         * The Multiplayer service
         * @type multiplayer
         */
        this.multiplayer = new wrapper._pio.multiplayer(channel);

        /**
         * The GameRequests service
         * @type gameRequests
         */
        this.gameRequests = new wrapper._pio.gameRequests(channel);

        /**
         * The Achievements service
         * @type achievements
         */
        this.achievements = new wrapper._pio.achievements(channel);

        /**
         * The PlayerInsight service
         * @type playerInsight
         */
        this.playerInsight = new wrapper._pio.playerInsight(channel);

        /**
         * The OneScore service
         * @type oneScore
         */
        this.oneScore = new wrapper._pio.oneScore(channel);

        /**
         * The Leaderboards service
         * @type leaderboards
         */
        this.leaderboards = new wrapper._pio.leaderboards(
          channel,
          this.connectUserId
        );
         ...
         * @type notifications
         */
        this.notifications = new wrapper._pio.notifications(channel);

        /**
         * The PlayerIO Publishing Network service
         * @type publishingNetwork
         */
        this.publishingNetwork = new wrapper._pio.publishingNetwork(
          channel,
          this.connectUserId
        );
      };
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.gameFS = function (gameId, redirectMap) {
        ...
        this.getUrl = function (path, secure) {
          ...
          if (map) {
            return (
              (secure ? map.secureBaseUrl : map.baseUrl) +
              (map["." + path] || path)
            );
          } else {
            return (
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.gameRequests = function (channel) {
        var _playCodes = [];
        ...
        this.send = function (
          ...
          channel.gameRequestsSend(
            requestType,
            wrapper._pio.convertToKVArray(requestData),
            requestRecipients,
            successCallback,
            errorCallback,
            function (result) {
       ...
       */
      wrapper._pio.gameRequest = function (
        id,
        type,
        senderUserId,
        created,
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.errorLog = function (channel) {
        /**
        ...
        this.writeError = function (error, details, stacktrace, extradata) {
          channel.writeError(
            ...
            details,
            stacktrace,
            wrapper._pio.convertToKVArray(extradata)
          );
        };
      };
    })(this);
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.quickConnect = function () {
        /**
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs
         */
        this.simpleGetCaptcha = function (
          gameId,
          width,
          height,
          successCallback,
          ...
          var channel = new wrapper._pio.channel();
          channel.simpleGetCaptcha(
            gameId,
            width,
            height,
            successCallback,
            errorCallback,
            function (result) {
              return new wrapper._pio.simpleGetCaptchaOutput(
                result.captchakey,
                result.captchaimageurl
              );
            }
          );
        };
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs
         */
        this.simpleRecoverPassword = function (
          gameId,
          usernameOrEmail,
          successCallback,
          errorCallback
       ...
       */
      wrapper._pio.simpleGetCaptchaOutput = function (
        captchaKey,
        captchaImageUrl
      ) {
      ...
      wrapper.PlayerIO.quickConnect = new wrapper._pio.quickConnect();
    })(this);
    (function (wrapper) {
      /**
       ...
       * </listing>
       */
      wrapper._pio.payVault = function (channel) {
        ...
        this.has = function (itemKey) {
          if (currentVersion == null) {
            throw new wrapper.PlayerIOError(
              wrapper.PlayerIOErrorCode.VaultNotLoaded,
          ...
          channel.payVaultReadHistory(
            ...
            function (result) {
              ...
              for (var i = 0; i != result.entries.length; i++) {
                ...
                arr.push(
                  new wrapper._pio.payVaultHistoryEntry(
                    item.type,
                    item.amount,
                    item.timestamp,
                    item.itemkeys || [],
        ...
        this.buy = function (
          ...
          channel.payVaultBuy(
            wrapper._pio.convertBuyItems(items),
            storeItems,
            null,
            successCallback,
            errorCallback,
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs
         */
        this.getBuyCoinsInfo = function (
          provider,
          purchaseArguments,
          successCallback,
          errorCallback
        ) {
          channel.payVaultPaymentInfo(
            provider,
            wrapper._pio.convertToKVArray(purchaseArguments),
            null,
            successCallback,
            errorCallback,
            function (result) {
              return wrapper._pio.convertFromKVArray(result.providerarguments);
            }
          );
        };
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs
         */
        this.getBuyDirectInfo = function (
          provider,
          purchaseArguments,
          items,
          successCallback,
          ...
          if (content != null) {
            ...
            if (content.items && content.items.length) {
              for (var i = 0; i != content.items.length; i++) {
                ...
                var obj = (self.items[i] = new wrapper._pio.vaultItem(
                  item.id,
                  item.itemkey,
                  new Date().setTime(item.purchasedate)
                ));

                wrapper._pio.bigDBDeserialize(item.properties, obj, true);
              }
            }
          }
        }
      ...
      wrapper._pio.convertBuyItems = function (items) {
        ...
        for (var i = 0; i != items.length; i++) {
          ...
          results.push({
            itemkey: itemKey,
            payload: wrapper._pio.compareForChanges(
              { itemkey: itemKey },
              items[i],
              true,
              true
       ...
       */
      wrapper._pio.vaultItem = function (id, itemKey, purchaseDate) {
       ...
       */
      wrapper._pio.payVaultHistoryEntry = function (
        type,
        amount,
        timestamp,
        itemkeys,
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.bigDB = function (channel) {
        var self = this;
        var queuedSaves = [];
        /**
        ...
        this.createObject = function (
          ...
          var properties = wrapper._pio.compareForChanges(
            {},
            obj || {},
            true,
            true
          ...
          channel.createObjects(
            ...
            function (results) {
              if (results.objects.length == 1) {
                return new wrapper._pio.databaseobject(
                  self,
                  table,
            ...
            function (results) {
              if (results.objects.length == 1) {
                return results.objects[0] == null
                  ? null
                  : new wrapper._pio.databaseobject(
                      self,
                      table,
                      results.objects[0].key,
                      results.objects[0].version,
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs
         */
        this.loadKeysOrCreate = function (
          table,
          keys,
          successCallback,
          errorCallback
          ...
          channel.createObjects(
            ...
            function (results) {
              ...
              for (var i = 0; i != results.objects.length; i++) {
                var obj = results.objects[i];
                ret[i] = new wrapper._pio.databaseobject(
                  self,
                  table,
                  obj.key,
                  obj.version,
        ...
        this.loadRange = function (
          ...
          channel.loadIndexRange(
            table,
            index,
            getIndexValue(indexPath, start),
            getIndexValue(indexPath, stop),
            limit,
            successCallback,
            errorCallback,
            function (results) {
        ...
        this.saveChanges = function (
          ...
          for (var i in objects) {
            ...
            var changes = wrapper._pio.compareForChanges(
              fullOverwrite ? {} : obj._internal_("get-dbCurrent"),
              obj,
              true,
              true
            );
            if (fullOverwrite || changes.length > 0) {
              changesets.push({
                ...
                onlyifversion: useOptimisticLock
                  ? obj._internal_("get-version")
                  : null,
                changes: changes,
              });
            }
          ...
          queuedSaves.push({
            ...
            execute: function () {
              ...
              if (self.changesets.length > 0) {
                channel.saveObjectChanges(
                  wrapper._pio.LockType.LockAll,
                  self.changesets,
                  createIfMissing,
                  successCallback,
                  function (e) {
        ...
        function getIndexValue(values, extraValue) {
          ...
          for (var i = 0; i != values.length; i++) {
            ...
            switch (typeof value) {
              case "boolean":
                result.push({
                  valuetype: wrapper._pio.ValueType.Bool,
                  bool: value,
                });
                break;
              case "string":
                result.push({
                  valuetype: wrapper._pio.ValueType.String,
                  string: value,
                });
                break;
              case "number":
                ...
                if (isFloatingPoint) {
                  result.push({
                    valuetype: wrapper._pio.ValueType.Double,
                    double: value,
                  });
                } else {
                  if (value > -2147483648 && value < 2147483647) {
                    // INTEGER RANGE
                    result.push({
                      valuetype: wrapper._pio.ValueType.Int,
                      int: value,
                    });
                  } else if (value > 0 && value < 4294967295) {
                    // uint
                    result.push({
                      valuetype: wrapper._pio.ValueType.UInt,
                      uint: value,
                    });
                  } else {
                    // long
                    result.push({
                      valuetype: wrapper._pio.ValueType.Long,
                      long: value,
                    });
                  }
                }
              ...
              case "object": // date, object & array
                if (value.getTime && value.getMilliseconds) {
                  // date
                  result.push({
                    valuetype: wrapper._pio.ValueType.DateTime,
       ...
       */
      wrapper._pio.databaseobject = function (
        owner,
        table,
        key,
        version,
        ...
        var dbCurrent = {};
        var isSaving = false;
        wrapper._pio.bigDBDeserialize(properties, dbCurrent, true);
        ...
        this._internal_ = function (method, arg1) {
          switch (method) {
            ...
            case "change-dbCurrent":
              wrapper._pio.bigDBDeserialize(arg1, dbCurrent, true);
          }
        };

        wrapper._pio.bigDBDeserialize(properties, this, true);
      };

      wrapper._pio.compareForChanges = function (
        original,
        current,
        isObject,
        isRoot
                ...
                if (value != valueOriginal) {
                  if (isFinite(value) && Math.floor(value) === value) {
                    if (value >= -2147483648 && value <= 2147483647) {
                      ...
                      prop.value = {
                        valuetype: wrapper._pio.ValueType.ULong,
                        ulong: value,
                      };
                    } else {
                      prop.value = {
              ...
              case "object": // date, object & array
                if (value.getTime && value.getMilliseconds) {
                  ...
                  if (value._circular_reference_finder_) {
                    throw new wrapper.PlayerIOError(
                      wrapper.PlayerIOErrorCode.CircularReference,
                  ...
                  if (valueIsArray && value.bytearray) {
                    ...
                    prop.value = {
                      valuetype: wrapper._pio.ValueType.ByteArray,
                      bytearray: bytes,
                    };
                  } else {
                    var subChanges = wrapper._pio.compareForChanges(
                      valueOriginal,
                      value,
                      !valueIsArray,
                      false
                    ...
                    if (valueIsArray) {
                      prop.value = {
                        valuetype: wrapper._pio.ValueType.Array,
                        arrayproperties: subChanges,
                      };
                    } else {
                      prop.value = {
                        valuetype: wrapper._pio.ValueType.Obj,
      ...
      wrapper._pio.bigDBDeserialize = function (properties, target, isObject) {
        for (var x in properties) {
          ...
          if (val) {
            switch (val.valuetype || 0) {
              case wrapper._pio.ValueType.String:
                target[key] = val.string || "";
                break;
              case wrapper._pio.ValueType.Int:
                target[key] = val.int || 0;
                break;
              case wrapper._pio.ValueType.UInt:
                target[key] = val.uint || 0;
                break;
              case wrapper._pio.ValueType.Long:
                target[key] = val.long || 0;
                break;
              case wrapper._pio.ValueType.Bool:
                target[key] = val.bool || 0;
                break;
              case wrapper._pio.ValueType.Float:
                target[key] = val.float || 0.0;
                break;
              case wrapper._pio.ValueType.Double:
                target[key] = val.double || 0.0;
                break;
              case wrapper._pio.ValueType.ByteArray:
                target[key] = val.bytearray;
                target[key].bytearray = true;
                break;
              case wrapper._pio.ValueType.DateTime:
                target[key] = new Date(val.datetime || 0);
                break;
              case wrapper._pio.ValueType.Array:
                var arr = target[key] instanceof Array ? target[key] : [];
                wrapper._pio.bigDBDeserialize(val.arrayproperties, arr, false);
                target[key] = arr;
                break;
              case wrapper._pio.ValueType.Obj:
                var obj = typeof target[key] == "object" ? target[key] : {};
                wrapper._pio.bigDBDeserialize(val.objectproperties, obj, true);
                target[key] = obj;
                break;
            }
          } else {
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.multiplayer = function (channel) {
        var self = this;
        /**
        ...
        this.createRoom = function (
          ...
          channel.createRoom(
            ...
            roomType,
            visible,
            wrapper._pio.convertToKVArray(roomData),
            self.developmentServer != null,
            successCallback,
            errorCallback,
            function (result) {
        ...
        this.joinRoom = function (
          ...
          clearTimeout(
            setTimeout(function () {
              successCallback(new wrapper._pio.connection());
            }, 10000)
          );
          var originalStack = new Error();
          channel.joinRoom(
            roomId,
            wrapper._pio.convertToKVArray(joinData),
            self.developmentServer != null,
            self.useSecureConnections,
            function () {},
            errorCallback,
            function (result) {
              return new wrapper._pio.connection(
                originalStack,
        ...
        this.listRooms = function (
          ...
          channel.listRooms(
            roomType,
            wrapper._pio.convertToKVArray(searchCriteria),
            resultLimit,
            resultOffset,
            self.developmentServer != null,
            successCallback,
            ...
            function (result) {
              ...
              if (result.rooms && result.rooms.length > 0) {
                for (var i = 0; i != result.rooms.length; i++) {
                  ...
                  arr.push(
                    new wrapper._pio.roomInfo(
                      item.id,
                      item.roomtype,
                      item.onlineusers,
                      wrapper._pio.convertFromKVArray(item.roomdata)
                    )
                  );
                }
              }
      ...
      wrapper._pio.websocketConnection = function (
        talkBinary,
          ...
          if (connected) {
            if (talkBinary) {
              ...
              fr.onloadend = function () {
                ...
                onMessage(
                  serializer.deserializeMessage(bytes, 0, bytes.length)
                );
              };
              fr.readAsArrayBuffer(msg.data);
            } else {
              var bytes = wrapper._pio.base64decode(msg.data);
        ...
        this.sendMessage = function (message) {
          ...
          if (talkBinary) {
            ...
            var str = wrapper._pio.base64encode(serialized);
            socket.send(str);
          }
        };
      };
       ...
       */
      wrapper._pio.connection = function (
        originalStack,
        developmentServer,
        useSecureConnections,
        endpoints,
        ...
        var browsers = /(WebKit|Firefox|Trident)\/([0-9]+)/gi.exec(
          navigator.userAgent
        );
        var engine = browsers && browsers.length >= 3 ? browsers[1] : null;
        var version =
          browsers && browsers.length >= 3 ? parseInt(browsers[2]) : null;
        var supportsBinaryWebsockets =
          (window.FileReader && engine == "WebKit" && version >= 535) ||
          (engine == "Firefox" && version >= 11) ||
          (engine == "Trident" && version >= 6);
        ...
        if (supportsBinaryWebsockets) {
          connect(function (
            ...
            return new wrapper._pio.websocketConnection(
              true,
              useSecureConnections,
              endpoint,
              connectTimeout,
          ...
          wrapper._pio.isFlashFallbackEnabled(function (enabled) {
            if (enabled) {
              connect(function (
                useSecureConnections,
                endpoint,
                ...
                return new wrapper._pio.flashSocketConnection(
                  endpoint,
                  connectTimeout,
                  onConnect,
                  onDisconnect,
        ...
        function connect(createSocket) {
          ...
          if (developmentServer) {
            ...
            for (var i = 0; i != endpoints.length; i++) {
              endpointStrings.push(
                endpoints[i].address + ":" + endpoints[i].port
              );
            }
          }
              ...
              var s = createSocket(
                ...
                function (connected, reason) {
                  ...
                  if (connected) {
                    ...
                    wrapper._pio.debugLog(
                      "Unable to connect to endpoint: " +
                        endpoint +
                        '. reason: "' +
                        reason +
                        (endpointStrings.length > 0
                  ...
                  if (self.connected) {
                    ...
                    setTimeout(function () {
                      for (var i = 0; i != disconnectCallbacks.length; i++) {
                        wrapper._pio.runCallback(
                          disconnectCallbacks[i].callback,
                          self,
                          disconnectCallbacks[i].stackSource
                        );
                      }
                    }, 100);
                  }
                  ...
                  if (waitingForJoinResult) {
                    if (msg.type == "playerio.joinresult") {
                      ...
                      if (!msg.getBoolean(0)) {
                        ...
                        wrapper._pio.runCallback(successCallback, self, null);
                      }
                    } else {
                      wrapper._pio.handleError(
                        originalStack,
        ...
        this.createMessage = function (type) {
          var msg = new wrapper._pio.message(type);
          for (var i = 1; i < arguments.length; i++) {
            msg.add(arguments[i]);
          }
          return msg;
       ...
       * </listing>
       */
      wrapper._pio.message = function (type) {
        var self = this;
        var types = [];
        var objects = [];
        ...
        this.add = function () {
          for (var i = 0; i < arguments.length; i++) {
            ...
            switch (typeof value) {
              ...
              default:
                throw wrapper._pio.error(
                  "The type of the value (" + value + ") cannot be inferred"
                );
            }
          }
        };
        ...
        this.addULong = function (value) {
          //Boundary is rounded because max_ulong can't be accurately represented as double
          add(
            value >= 0 && value <= 18446744073709550000,
            value,
            entryType_UnsignedLong,
            "an unsigned long (64bit)"
          );
        };
        ...
        this.addDouble = function (value) {
          add(
            ...
            Number(value),
            entryType_Double,
            "a double floating point value (64bit)"
          );
        };
        ...
        function add(check, value, type, errorMessage) {
          if (check) {
            ...
            throw wrapper._pio.error(
              "The given value (" + value + ") is not " + errorMessage
            );
          }
        }
       ...
       */
      wrapper._pio.roomInfo = function (id, roomType, onlineUsers, roomData) {
        /** The id of the room
         * @type string
         */
        this.id = id;
      ...
      wrapper._pio.byteWriter = function () {
        this.bytes = [];
        ...
        this.writeTagWithLength = function (length, topPattern, bottomPattern) {
          if (length > 63 || length < 0) {
            this.writeBottomPatternAndBytes(
              bottomPattern,
              wrapper._pio.binaryserializer.bytesFromInt(length)
            );
          } else {
      ...
      wrapper._pio.messageSerializer = function () {
        ...
        this.serializeMessage = function (message) {
          var writer = new wrapper._pio.byteWriter();
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
          ...
          for (var i = 0; i != message.length; i++) {
            ...
            switch (message._internal_("get-types")[i]) {
              ...
              case entryType_UnsignedInteger:
                writer.writeBottomPatternAndBytes(
                  unsignedIntegerBottomPattern,
                  wrapper._pio.binaryserializer.bytesFromUInt(value)
                );
                break;
              case entryType_Long:
                writer.writeLongPattern(
                  shortLongBottomPattern,
                  longBottomPattern,
                  wrapper._pio.binaryserializer.bytesFromLong(value)
                );
                break;
              case entryType_UnsignedLong:
                writer.writeLongPattern(
                  ...
                  wrapper._pio.binaryserializer.bytesFromULong(value)
                );
                break;
              ...
              case entryType_Double:
                ...
                writer.writeBytes(
                  wrapper._pio.binaryserializer.bytesFromDouble(value)
                );
                break;
              case entryType_Float:
                writer.writeByte(floatPattern);
                var fb = wrapper._pio.binaryserializer.bytesFromFloat(value);
                writer.writeBytes(fb);
                break;
              case entryType_Boolean:
                writer.writeByte(
                  value ? booleanTruePattern : booleanFalsePattern
                );
                break;
            }
          }
          ...
          while (position < end) {
            ...
            switch (pattern) {
              ...
              case byteArrayBottomPattern:
                ...
                var jump = length;
                length = wrapper._pio.binaryserializer.intFromBytes(
                  bytes,
                  position,
                  length
                );
            ...
            switch (pattern) {
              ...
              case stringTopPattern:
                if (output == null) {
                  output = new wrapper._pio.message(
                    wrapper._pio.binaryserializer.stringFromBytes(
                      bytes,
                      position,
                      length
                    )
                ...
                break;
              case integerBottomPattern:
                value = wrapper._pio.binaryserializer.intFromBytes(
                  bytes,
                  position,
                  length
                );
              ...
              case unsignedIntegerBottomPattern:
                output.addUInt(
                  wrapper._pio.binaryserializer.uintFromBytes(
                    bytes,
                    position,
                    length
                  )
              ...
              case longBottomPattern:
                output.addLong(
                  wrapper._pio.binaryserializer.longFromBytes(
                    bytes,
                    position,
                    length
                  )
              ...
              case unsignedLongBottomPattern:
                output.addULong(
                  wrapper._pio.binaryserializer.ulongFromBytes(
                    bytes,
                    position,
                    length
                  )
              ...
              case doublePattern:
                output.addDouble(
                  wrapper._pio.binaryserializer.doubleFromBytes(
                    bytes,
                    position,
                    length
                  )
              ...
              case floatPattern:
                output.addFloat(
                  wrapper._pio.binaryserializer.floatFromBytes(
                    bytes,
                    position,
                    length
                  )
      ...
      wrapper._pio.binaryserializer = {
        pow2: function (n) {
          return n >= 0 && n < 31
            ? 1 << n
            : this.pow2[n] || (this.pow2[n] = Math.pow(2, n));
        },
        ...
        _floatEncode: function (value, mantSize, expSize) {
          ...
          if (value === 0) {
            ...
            if (exponent >= eMin && exponent <= eMax) {
              mantissa = Math.floor(
                (value * this.pow2(-exponent) - 1) * this.pow2(mantSize)
              );
              exponent += eMax;
            } else {
      ...
      wrapper._pio.base64encode = function (bytes) {
        var output = [];
      ...
      wrapper._pio.base64decode = function (string) {
        var output = [];
        for (var c = 0; c < string.length; c++) {
          // pick the 4 characters representing 3 bytes
          var chr1 = inverseCodex[string.charCodeAt(c)];
          var chr2 =
            ++c < string.length ? inverseCodex[string.charCodeAt(c)] : 64;
          var chr3 =
            ++c < string.length ? inverseCodex[string.charCodeAt(c)] : 64;
          var chr4 =
            ++c < string.length ? inverseCodex[string.charCodeAt(c)] : 64;
    ...
    (function (wrapper) {
       ...
       *</listing>
       */
      wrapper._pio.achievements = function (channel) {
        var currentVersion = null;
        ...
        this.addOnComplete = function (onCompleteHandler) {
          if (
            ...
            onCompleteHandler.length == 1
          ) {
            self.onCompleteHandlers.push(onCompleteHandler);
          } else {
            throw new wrapper.PlayerIOError(
              wrapper.PlayerIOErrorCode.InvalidArgument,
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs.
         */
        this.progressSet = function (
          achievementId,
          progress,
          successCallback,
          errorCallback
        ) {
          channel.achievementsProgressSet(
            achievementId,
            progress,
            successCallback,
            errorCallback,
        ...
        this.progressAdd = function (
          ...
          channel.achievementsProgressAdd(
            achievementId,
            progressDelta,
            successCallback,
            errorCallback,
        ...
        this.progressMax = function (
          ...
          channel.achievementsProgressMax(
            achievementId,
            progress,
            successCallback,
            errorCallback,
        ...
        this.progressComplete = function (
          ...
          channel.achievementsProgressComplete(
            achievementId,
            successCallback,
            errorCallback,
            function (result) {
        ...
        function update(achievement, completednow) {
          ...
          var ach = new wrapper._pio.achievement(
       ...
       */
      wrapper._pio.achievement = function (
        id,
        title,
        description,
        imageUrl,
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.playerInsight = function (channel) {
        ...
        this.setSegments = function (segments, successCallback, errorCallback) {
          channel.playerInsightSetSegments(
            wrapper._pio.convertToSegmentArray(segments),
            successCallback,
            errorCallback,
            function (result) {
              setState(result.state);
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs.
         */
        this.trackInvitedBy = function (
          invitingUserId,
          invitationChannel,
          successCallback,
          errorCallback
        ) {
          channel.playerInsightTrackInvitedBy(
            invitingUserId,
            invitationChannel,
            successCallback,
            errorCallback,
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs.
         */
        this.trackExternalPayment = function (
          currency,
          amount,
          successCallback,
          errorCallback
        ) {
          channel.playerInsightTrackExternalPayment(
            currency,
            amount,
            successCallback,
            errorCallback,
        ...
        this.sessionKeepAlive = function (successCallback, errorCallback) {
          channel.playerInsightSessionKeepAlive(
            successCallback,
            errorCallback,
            function (result) {}
          );
        ...
        function setState(state) {
          ...
          self.segments = wrapper._pio.convertFromKVArray(state.segments);
        }
      };
    })(this);
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.leaderboards = function (channel, connectUserId) {
        /**
        ...
        this.set = function (
          ...
          channel.leaderboardsSet(
            ...
            errorCallback,
            function (result) {
              var entry = new wrapper._pio.leaderboardEntry(
                result.leaderboardentry.userid,
                result.leaderboardentry.rank,
                result.leaderboardentry.score
              );
              return entry;
            }
          );
          ...
          channel.leaderboardsGet(
            ...
            function (result) {
              if (
                ...
                result.leaderboardentry.length == 0
              ) {
                return [];
              }
              var entries = [];
              for (var i = 0; i < result.leaderboardentry.length; i++) {
                entries.push(
                  new wrapper._pio.leaderboardEntry(
                    result.leaderboardentry[i].userid,
                    result.leaderboardentry[i].rank,
                    result.leaderboardentry[i].score
                  )
                );
              }
              return entries;
       ...
       */
      wrapper._pio.leaderboardEntry = function (userId, rank, score) {
        /** The connectUserId
         * @type String
         */
        this.userId = userId;
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.oneScore = function (channel) {
         ...
         */
        this.refresh = function (successCallback, errorCallback) {
          channel.oneScoreRefresh(
            successCallback,
            errorCallback,
            function (result) {
              var score = new wrapper._pio.oneScoreValue(
                result.onescore.percentile,
                result.onescore.score,
                result.onescore.toprank
              );
        ...
        this.load = function (userIds, successCallback, errorCallback) {
          ...
          channel.oneScoreLoad(
            ...
            function (result) {
              if (
                result == null ||
                result.onescores == null ||
                result.onescores.length == 0
              ) {
                return [];
              }
              var scores = [];
              ...
              for (var i = 0; i < userIds.length; i++) {
                ...
                if (score && userIds[i] == score.userid) {
                  scores.push(
                    new wrapper._pio.oneScoreValue(
                      score.percentile,
                      score.score,
                      score.toprank
                    )
       ...
       */
      wrapper._pio.oneScoreValue = function (percentile, score, topRank) {
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.notifications = function (channel) {
          ...
          if (result.version != version) {
            ...
            if (result.endpoints) {
              for (var i = 0; i != result.endpoints.length; i++) {
                ...
                list[i] = new wrapper._pio.notificationEndpoint(
                  e.type,
                  e.identifier,
                  wrapper._pio.convertFromKVArray(e.configuration),
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs.
         */
        this.registerEndpoint = function (
          endpointType,
          identifier,
          configuration,
          enabled,
          ...
          if (
            ...
            !equalConfiguration(current.configuration, configuration)
          ) {
            channel.notificationsRegisterEndpoints(
              version,
              [
                {
                  type: endpointType,
                  identifier: identifier,
                  configuration: wrapper._pio.convertToKVArray(configuration),
                  enabled: enabled,
                },
              ],
              successCallback,
        ...
        this.toggleEndpoints = function (
          ...
          if (ids.length > 0) {
            channel.notificationsToggleEndpoints(
              version,
              ids,
              enable ? true : false,
              successCallback,
        ...
        this.deleteEndpoints = function (
          ...
          var ids = getIds(endpoints);
          if (ids.length > 0) {
            channel.notificationsDeleteEndpoints(
              version,
              ids,
              successCallback,
              errorCallback,
        ...
        this.send = function (notifications, successCallback, errorCallback) {
          ...
          for (var i = 0; i != notifications.length; i++) {
            ...
            if (
              (n.recipient + "").length == 0 ||
              (n.endpointtype + "").length == 0
            ) {
              console.log("error");
            }
            for (var x in s) {
       ...
       */
      wrapper._pio.notificationEndpoint = function (
        type,
        identifier,
        configuration,
        enabled
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.publishingNetwork = function (channel, connectUserId) {
        var self = this;
        /**
         * Access to the PlayerIO Publishing Network profiles
         * @type publishingNetworkProfiles
         */
        this.profiles = new wrapper._pio.publishingNetworkProfiles(channel);

        /**
         * Access to PlayerIO Publishing Network payments
         * @type publishingNetworkPayments
         */
        this.payments = new wrapper._pio.publishingNetworkPayments(channel);

        /**
         * Access to PlayerIO Publishing Network relations
         * @type publishingNetworkRelations
         */
        this.relations = new wrapper._pio.publishingNetworkRelations(
          channel,
          connectUserId,
          this
        );
          ...
          channel.socialRefresh(
            ...
            function (result) {
              ...
              if (typeof wrapper._pio.friendLookup == "undefined") {
                ...
                wrapper._pio.blockedLookup = {};
              }

              var fl =
                wrapper._pio.friendLookup[self.profiles.myProfile.userId];
              var bl =
              ...
              for (var i = 0; i != result.friends.length; i++) {
                var f = new wrapper._pio.publishingNetworkProfile(
                  result.friends[i]
                );
      ...
      wrapper._pio.showDialog = function (
        dialog,
        channel,
        args,
        closedCallback
        ...
        if (typeof window.PublishingNetwork == "undefined") {
          throw new wrapper.PlayerIOError(
            wrapper.PlayerIOErrorCode.PublishingNetworkNotAvailable,
    ...
    (function (wrapper) {
       ...
       * </listing>
       */
      wrapper._pio.publishingNetworkPayments = function (channel) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        var self = this;
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs.
         */
        this.showBuyCoinsDialog = function (
          coinAmount,
          purchaseArguments,
          successCallback,
          errorCallback
          ...
          channel.payVaultPaymentInfo(
            ...
            null,
            function (result) {
              wrapper._pio.showDialog("buy", channel, result, function (r) {
                if (r.error) {
                  errorCallback(
                    new wrapper.PlayerIOError(
                      wrapper.PlayerIOErrorCode.GeneralError,
         ...
         * @param {function(wrapper.PlayerIOError)} errorCallback Callback function that will be called if an error occurs.
         */
        this.showBuyItemsDialog = function (
          items,
          purchaseArguments,
          successCallback,
          errorCallback
    ...
    (function (wrapper) {
       ...
       */
      wrapper._pio.publishingNetworkProfiles = function (channel) {
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        var self = this;
              ...
              for (var i = 0; i != userIds.length; i++) {
                ...
                for (var x = 0; x != result.profiles.length; x++) {
                  ...
                  if (user && user.userid == userId) {
                    arr[i] = new wrapper._pio.publishingNetworkProfile(
                      result.profiles[x]
                    );
                    break;
                  }
       ...
       */
      wrapper._pio.publishingNetworkProfile = function (profile) {
    ...
    (function (wrapper) {
       ...
       */
      wrapper._pio.publishingNetworkRelations = function (
        channel,
        connectUserId,
        publishingNetwork
      ) {
        ...
        this.isFriend = function (userId) {
          if (
            typeof wrapper._pio.friendLookup != "undefined" &&
            typeof wrapper._pio.friendLookup[connectUserId] != "undefined"
          ) {
            return wrapper._pio.friendLookup[connectUserId][userId] || false;
          } else {
            throw new wrapper.PlayerIOError(
              wrapper.PlayerIOErrorCode.PublishingNetworkNotLoaded,
              "Cannot access profile, friends, ignored before Publishing Network has been loaded. Please refresh Publishing Network first"
            );
          }
        };
        ...
        this.isBlocked = function (userId) {
          if (
            typeof wrapper._pio.blockedLookup != "undefined" &&
            typeof wrapper._pio.blockedLookup[connectUserId] != "undefined"
          ) {
            return wrapper._pio.blockedLookup[connectUserId][userId] || false;
          } else {
            throw new wrapper.PlayerIOError(
              wrapper.PlayerIOErrorCode.PublishingNetworkNotLoaded,
              "Cannot access profile, friends, ignored before Publishing Network has been loaded. Please refresh Publishing Network first"
