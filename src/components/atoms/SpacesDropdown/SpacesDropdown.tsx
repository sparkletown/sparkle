...
type SpacesDropdownPortal = { template?: PortalTemplate; name: string };
export interface SpacesDropdownProps {
  ...
  fieldName: string;
  error?: FieldError;
  portals: Record<string, WithId<AnyVenue>>;
}
...
export const SpacesDropdown: React.FC<SpacesDropdownProps> = ({
  ...
  const renderedOptions = useMemo(() => {
    const options = Object.values(filteredPortals).map(({ name, template }) => {
