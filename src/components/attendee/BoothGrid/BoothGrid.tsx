...
export const BoothGrid: React.FC<BoothGridProps> = ({ space, user }) => {
  ...
  const tables: Table[] = useMemo(() => {
    ...
    return managedSpaces.map((managedSpace) => ({
      title: managedSpace.name,
      reference: managedSpace.id,
      capacity: 20,
    }));
  }, [isLoadingManagedSpaces, managedSpaces]);
  ...
  }, [tables, tableLocked, usersSeatedAtTables, onJoinClicked, space, user.id]);
