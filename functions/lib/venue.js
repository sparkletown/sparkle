...
exports.deleteScreeningRoomVideo = exports.upsertScreeningRoomVideo = exports.setVenueLiveStatus = exports.adminUpdateIframeUrl = exports.adminUpdateBannerMessage = exports.voteInPoll = exports.deleteVenue = exports.deleteTable = exports.updateTables = exports.updateVenueNG = exports.updateMapBackground = exports.updateVenue_v2 = exports.deleteRoom = exports.upsertRoom = exports.createVenue_v2 = exports.removeVenueOwner = exports.addVenueOwner = exports.setAuditoriumSections = void 0;
...
const checkUserIsOwner = async (venueId, uid) => {
    await admin
        ...
        if (!venue) {
            throw new https_1.HttpsError("internal", "No data returned");
        }
        ...
        if (venue.parentId) {
            const doc = await admin
                ...
                .get();
            if (!doc.exists) {
                throw new https_1.HttpsError("not-found", `Venue ${venueId} references missing parent ${venue.parentId}`);
            }
            ...
            if (!(parentVenue.owners && parentVenue.owners.includes(uid))) {
                throw new https_1.HttpsError("permission-denied", `User is not an owner of ${venueId} nor parent ${venue.parentId}`);
            }
        }
        throw new https_1.HttpsError("permission-denied", `User is not an owner of ${venueId}`);
    })
        .catch((err) => {
        throw new https_1.HttpsError("internal", `Error occurred obtaining venue ${venueId}: ${err.toString()}`);
    });
};
...
const checkIfUserHasVoted = async (venueId, pollId, userId) => {
    await admin
        ...
        .then((doc) => {
        if (!doc.exists) {
            throw new https_1.HttpsError("not-found", `Poll ${pollId} does not exist`);
        }
        const poll = doc.data();
        if (!poll) {
            throw new https_1.HttpsError("internal", "No data returned");
...
const createVenueData_v2 = (data, context) => {
    const venueData_v2 = Object.assign(Object.assign({ name: data.name, config: Object.assign(Object.assign({}, (Array.isArray(data.tables) && { tables: data.tables })), { landingPageConfig: {
        ...
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-ignore
        owners: [context.auth.token.user_id], showGrid: data.showGrid || false }, (data.showGrid && { columns: data.columns })), { template: data.template || VenueTemplate.partymap, rooms: [], createdAt: Date.now(), updatedAt: Date.now(), parentId: data.parentId || "", worldId: data.worldId, slug: data.slug });
...
const initializeVenueChatMessagesCounter = (venueRef, batch) => {
    ...
    for (let shardId = 0; shardId < VENUE_CHAT_MESSAGES_COUNTER_SHARDS_COUNT; shardId++) {
        batch.set(counterCollection.doc(shardId.toString()), { count: 0 });
    }
    batch.set(counterCollection.doc("sum"), { value: 0 });
};
exports.setAuditoriumSections = functions.https.onCall(async (data, context) => {
...
exports.addVenueOwner = functions.https.onCall(async (data, context) => {
    ...
    await (0, roles_1.addAdmin)(newOwnerId);
});
...
exports.createVenue_v2 = functions.https.onCall(async (data, context) => {
    ...
    if (venueExists) {
        throw new https_1.HttpsError("already-exists", `The slug ${data.slug} is already taken by a space within the world. Please try another.`);
    }
...
exports.upsertRoom = functions.https.onCall(async (data, context) => {
    ...
    if (!doc || !doc.exists) {
        throw new https_1.HttpsError("not-found", `Venue ${venueId} not found`);
    }
...
exports.deleteRoom = functions.https.onCall(async (data, context) => {
    ...
    if (index === -1) {
        throw new https_1.HttpsError("not-found", `${taxonomy_1.ROOM_TAXON.capital} does not exist`);
    }
    else {
...
exports.updateTables = functions.https.onCall((data, context) => {
    (0, assert_1.assertValidAuth)(context);
    const isValidVenueId = (0, venue_1.checkIfValidVenueId)(data.venueId);
    if (!isValidVenueId) {
        throw new https_1.HttpsError("invalid-argument", `venueId is not a valid venue id`);
    }
    ...
    return admin.firestore().runTransaction(async (transaction) => {
        ...
        if (!spaceDoc.exists) {
            throw new https_1.HttpsError("not-found", `venue ${data.venueId} does not exist`);
        }
        const space = spaceDoc.data();
        if (!space) {
            throw new https_1.HttpsError("internal", "data not found");
        }
        const spaceTables = (space.config && space.config.tables) || data.defaultTables;
        const currentTableIndex = spaceTables.findIndex((table) => table.reference === data.newTable.reference);
...
exports.deleteTable = functions.https.onCall(async (data, context) => {
    ...
    if (!doc || !doc.exists) {
        throw new https_1.HttpsError("not-found", `Venue ${spaceId} not found`);
    }
    const docData = doc.data();
    if (!docData) {
        throw new https_1.HttpsError("internal", "data not found");
    ...
    if (!docData.config || !docData.config.tables) {
        ...
        admin
            ...
            .collection("venues")
            .doc(spaceId)
            .update(Object.assign(Object.assign({}, docData), { config: Object.assign(Object.assign({}, docData.config), { tables: defaultTables }) }));
        return;
    }
    if (index === -1) {
        throw new https_1.HttpsError("not-found", `Table does not exist`);
    }
    else {
...
// @debt extract this into a new functions/chat backend script file
exports.voteInPoll = functions.https.onCall(async ({ venueId, pollVote }, context) => {
    (0, assert_1.assertValidAuth)(context);
    ...
    try {
        ...
        throw new https_1.HttpsError("unavailable", `User ${userId} has voted in ${pollId} Poll`, error);
    }
});
exports.adminUpdateBannerMessage = functions.https.onCall(async (data, context) => {
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
...
exports.setVenueLiveStatus = functions.https.onCall(async (data, context) => {
    ...
    if (typeof data.isLive !== "boolean") {
        throw new https_1.HttpsError("invalid-argument", `isLive is not a boolean`);
    }
    const update = {
...
exports.upsertScreeningRoomVideo = functions.https.onCall(async (data, context) => {
...
exports.deleteScreeningRoomVideo = functions.https.onCall(async (data, context) => {
