...
const getUsersWithVisits = async (venueIdsArray) => {
    const dto = await (0, lodash_1.chunk)(venueIdsArray, 10)
        .map(async (idsArray) => {
        return await admin
            .firestore()
            .collection("users")
            ...
            .get()
            .then((usersSnapshot) => usersSnapshot.docs.map(async (userDoc) => {
            const user = Object.assign(Object.assign({}, userDoc.data()), { id: userDoc.id });
            const visits = await userDoc.ref
                .collection("visits")
                .get()
                .then((visitsSnapshot) => visitsSnapshot.docs.map((visitDoc) => (Object.assign(Object.assign({}, visitDoc.data()), { id: visitDoc.id }))));
            return { user, visits };
        }));
    })
...
const generateAnalytics = async (data, context) => {
    var _a;
    (0, assert_1.assertValidAuth)(context);
    // eslint-disable-next-line @typescript-eslint/ban-ts-comment
    // @ts-ignore
    await checkIsAdmin(context.auth.token.user_id);
    const venueIdsArray = Array.isArray(data === null || data === void 0 ? void 0 : data.venueIds)
        ? data.venueIds
        : ((_a = data === null || data === void 0 ? void 0 : data.venueIds) !== null && _a !== void 0 ? _a : "").split(",");
    // TODO: CHECK IF ADMIN
    // TODO: extract this as a generic helper function?
    const usersWithVisits = await Promise.all(await getUsersWithVisits(venueIdsArray).then((res) => res.flat()));
    // TODO: extract this as a generic helper function?
    // Note: 100 is the max that can be requested with getUsers() per chunk
    const authUsers = await Promise.all((0, lodash_1.chunk)(usersWithVisits, 100).map(async (usersWithVisitsChunk) => {
    ...
    const authUsersById = authUsers.reduce((acc, authUser) => (Object.assign(Object.assign({}, acc), { [authUser.uid]: authUser })), {});
    ...
    const result = usersWithVisits
        ...
        const matchingUserIndex = arr.findIndex((item) => item.user.partyName === user.partyName);
        if (matchingUserIndex > 0) {
            const newArr = [...arr[matchingUserIndex].visits, ...visits].reduce((visitsArr, visit) => {
            ...
            arr[matchingUserIndex] = Object.assign(Object.assign({}, userWithVisits), { visits: newArr });
            return arr;
        }
        ...
        visitIds.map((visit) => fs.writeFileSync(allSpaceVisitsFileName, `${visit} \n`, { flag: "a" }));
        return {
            id,
            email,
            partyName,
    ...
    globalUniqueVisits.map((el) => fs.writeFileSync(uniqueVenuesVisitedFileName, `${el} \n`, { flag: "a" }));
    ...
    const allDataResult = result.map((user) => {
        ...
        const getVisitData = (visitName) => {
            const found = user.visits.find((visit) => visit.id === visitName ? visit : 0);
            return found
                ? found
                : {
                    timeSpent: 0,
        ...
        const formattedVisitColumns = globalUniqueVisits.map((visit) => {
            if (onlyVisitIds.includes(visit) && getVisitData(visit)) {
                ...
                return {
                    timeValue: timeSpent,
                    formattedTime: `${(0, time_1.formatSecondsAsHHMMSS)(timeSpent !== null && timeSpent !== void 0 ? timeSpent : 0)}`,
                };
            }
            return {};
        });
        const bodyVisitColumns = formattedVisitColumns.map((column) => (column && column.formattedTime) || "");
        const dto = [
            user.email,
            user.partyName,
            user.visits.length,
    ...
    [0, 1, 2].map(() => fs.writeFileSync(dataReportFileName, `\n`, { flag: "a" }));
    ...
    const averageVisitDataLine = [
        ...
        ...arrayOfResults.map((el) => `${(0, time_1.formatSecondsAsHHMMSS)(Math.round(el.totalValue / el.totalAmount))}`),
    ];
    ...
    const [uniqueVenuesVisitedFile] = await bucket.upload(uniqueVenuesVisitedFileName, {
