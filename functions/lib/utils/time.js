...
exports.formatSecondsAsHHMMSS = void 0;
/**
...
const secondsToDuration = (totalSeconds) => {
    ...
    const remainingSecondsWithoutHours = remainingSecondsWithoutDays % ONE_HOUR_IN_SECONDS;
    const minutes = Math.floor(remainingSecondsWithoutHours / ONE_MINUTE_IN_SECONDS);
    const remainingSecondsWithoutMinutes = remainingSecondsWithoutHours % ONE_MINUTE_IN_SECONDS;
    return { days, hours, minutes, seconds: remainingSecondsWithoutMinutes };
};
const formatSecondsAsHHMMSS = (secondsValue) => {
    const { hours = 0, minutes = 0, seconds = 0 } = secondsToDuration(secondsValue);
