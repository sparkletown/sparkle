...
import { addAdmin } from "./api/roles";
import { assertValidUrl, assertValidVenueId } from "./utils/assert";
import { createOAuth2Client } from "./utils/auth";
...
export const connectI4AOAuth = functions.https.onRequest(async (req, res) => {
  ...
  if (typeof venueId !== "string") {
    throw new HttpsError("invalid-argument", "venueId is not a string");
  }
  if (typeof returnOrigin !== "string") {
    throw new HttpsError("invalid-argument", "returnOrigin is not a string");
  }
