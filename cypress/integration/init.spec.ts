...
describe("initial test visiting plain /admin", () => {
  ...
  it("has a root element with only child being a blank page", () => {
    getRoot().should("have.length", 1);
    getRoot().children().first().should("have.attr", "data-bem", "BlankPage");
  });

  it("has blank page with 401 text in it", () => {
    getBlankPage().should("contain", "401");
  });
});
